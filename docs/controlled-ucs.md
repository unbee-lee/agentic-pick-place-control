...
| Unrecognised wording | Ask for supported input. The restricted demo parser may require cancelling and starting a new explicit-pair request. |
...
Clarification appears in a highlighted question panel with the original request and a focused answer field. The activity trail resets when a new arrangement request starts; replies keep the history for that request. Refresh and SSE reconnect restore the current question, target, status, and request trail from the server. Input stays disabled until that snapshot arrives. To start over while a clarification is pending, select Cancel request first.
...
LangGraph checkpoints retain the original request and clarification replies in memory. A reply resumes the same workflow by its identifier. The successful agentic output is the validated Target arrangement; deterministic application code handles delivery, progress, and terminal results.
...
Session/checkpoint state and duplicate tracking are in memory. Restart recovery, durable dispatch tracking, and late-result reconciliation remain integration work. Page refresh restores the current in-memory session; it does not reset or cancel an active command. A simulated result is not physical or visual verification.
